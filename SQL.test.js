...
test('SQL helper - build complex query with append', async t => {
  ...
  t.assert.deepStrictEqual(sql.values, [name, description, teamId, organizationId])
})
...
test('SQL helper - build complex query with map', async t => {
  ...
  t.assert.equal(values !== null, true)
  const sql = SQL`INSERT INTO users (id) VALUES (${values})`

  t.assert.equal(sql.text, 'INSERT INTO users (id) VALUES ($1,$2,$3)')
  t.assert.equal(sql.sql, 'INSERT INTO users (id) VALUES (?,?,?)')
  t.assert.equal(sql.debug, 'INSERT INTO users (id) VALUES (1,2,3)')
  t.assert.deepStrictEqual(sql.values, [1, 2, 3])
})
...
test('SQL helper - build complex query with glue - regression #13', async t => {
  ...
  t.assert.equal(sql.text, 'UPDATE teams SET name = $1 WHERE id IN ($2 , $3 , $4 )')
  t.assert.equal(sql.sql, 'UPDATE teams SET name = ? WHERE id IN (? , ? , ? )')
  t.assert.equal(
    sql.debug,
    `UPDATE teams SET name = '${name}' WHERE id IN (1 , 2 , 3 )`
  )
  t.assert.deepStrictEqual(sql.values, [name, 1, 2, 3])
})
...
test('glue works with quoteIdent - regression #77', async t => {
  ...
  t.assert.equal(sql.text, 'SELECT * FROM "tbl"')
  t.assert.equal(sql.sql, 'SELECT * FROM `tbl`')
  t.assert.equal(sql.debug, 'SELECT * FROM "tbl"')
  t.assert.deepStrictEqual(sql.values, [])
})
...
test('SQL helper - build complex query with append and glue', async t => {
  ...
  t.assert.deepStrictEqual(sql.values, [v1, v2, v3, v4, v5, v6, v7])
})
...
test('SQL helper - build string using append with and without unsafe flag', async t => {
  ...
  t.assert.equal(sql.values.length, 1)
  t.assert.ok(sql.values.includes(v2))
})
...
test('SQL helper - build string using append and only unsafe', async t => {
  ...
  const sql = SQL`TEST QUERY glue pieces FROM test WHERE test1 == test2`
  t.assert.equal(sql.text, 'TEST QUERY glue pieces FROM test WHERE test1 == test2')
...
test('SQL helper - handles js null values as valid `null` sql values', async t => {
  ...
  t.assert.equal(sql.text, 'UPDATE teams SET name = $1 WHERE id = $2')
  t.assert.equal(sql.sql, 'UPDATE teams SET name = ? WHERE id = ?')
  t.assert.equal(sql.debug, `UPDATE teams SET name = null WHERE id = ${id}`)
  t.assert.deepStrictEqual(sql.values, [name, id])
})

test('SQL helper - throws when building an sql string with an `undefined` value', async t => {
  t.assert.throws(() => SQL`UPDATE teams SET name = ${undefined}`)
})

test('empty append', async t => {
  const sql = SQL`UPDATE teams SET name = ${'team'}`.append()

  t.assert.equal(sql.text, 'UPDATE teams SET name = $1')
  t.assert.equal(sql.sql, 'UPDATE teams SET name = ?')
  t.assert.equal(sql.debug, "UPDATE teams SET name = 'team'")
  t.assert.deepStrictEqual(sql.values, ['team'])
})

test('inspect', async t => {
  const sql = SQL`UPDATE teams SET name = ${'team'}`
  t.assert.equal(util.inspect(sql), "SQL << UPDATE teams SET name = 'team' >>")
})
...
test('quoteIdent', async t => {
  t.test('simple', async t => {
    ...
    t.assert.equal(sql.text, 'UPDATE "teams" SET name = $1 WHERE id = $2')
    t.assert.equal(sql.sql, 'UPDATE `teams` SET name = ? WHERE id = ?')
    t.assert.equal(sql.debug, `UPDATE "teams" SET name = 'name' WHERE id = ${id}`)
    t.assert.deepStrictEqual(sql.values, [name, id])
  })
})
...
test('unsafe', async t => {
  ...
  t.assert.equal(sql.text, "UPDATE teams SET name = 'name' WHERE id = $1")
  t.assert.equal(sql.sql, "UPDATE teams SET name = 'name' WHERE id = ?")
  t.assert.equal(sql.debug, `UPDATE teams SET name = 'name' WHERE id = ${id}`)
  t.assert.deepStrictEqual(sql.values, [id])
})
...
test('should be able to append a SqlStatement within a template literal', t => {
  ...
  t.assert.equal(selectWithLiteralExpression.text, 'SELECT * FROM table')
  t.assert.equal(selectWithLiteralExpression.sql, 'SELECT * FROM table')
  t.assert.equal(selectWithLiteralExpression.debug, 'SELECT * FROM table')
})
...
test('should be able to use SQL.glue within template literal', t => {
  ...
  t.assert.deepStrictEqual(sql.values, ['A', 1, '2', 'three', 'Bee', 'Cee', 'Dee', 'B'])
})
...
test('should be able to use nested SQLStatements in template literal', t => {
  ...
  t.assert.deepStrictEqual(sql.values, ['A', 'B', 'C', 'D'])
})
...
test('examples in the readme work as expected', t => {
  {
    ...
    t.assert.equal(sql.text, 'UPDATE users SET name = $1 , email = $2 WHERE id = $3')
  }
  {
    const ids = [1, 2, 3]
    const value = 'test'
